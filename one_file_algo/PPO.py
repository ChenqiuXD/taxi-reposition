...
if __name__ == "__main__":
    ...
    if train_test=="train":
        ...
        for t in range(TOTAL_TIMESTEPS):
            ...
            next_obs, reward, done, terminate, _ = env.step(clipped_action)
        ...
        torch.save(pi_network.state_dict(), 'ppo_pi_network.pth')
        torch.save(v_network.state_dict(), 'ppo_v_network.pth')
        ...
        ax.grid(True)
        plt.savefig("ppo_season_reward.png")
        ...
        pi_network = PI_Network(obs_dim, action_dim, lower_bound, upper_bound)
        pi_network.load_state_dict(torch.load('ppo_pi_network.pth'))
        obs, info = env.reset()
        frames = []
        for _ in range(300):
            ...
            env.render()
            obs, reward, done, terminate,  _ = env.step(clipped_action)
        env.close()
        # save_frames_as_gif(frames, filename="pendulum_run.gif")
