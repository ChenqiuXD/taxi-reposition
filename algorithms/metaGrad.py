...
class metaAgent(BaseAgent):
    ...
    def learn(self):
        ...
        if self.buffer_high > self.batch_size:
            sample_index = np.random.choice( self.buffer_high-1, self.batch_size, replace=False ) # Cannot sample replicate samples. 
        ...
        for idx, state in enumerate(batch_state):
            ...
            L = self.get_L(agents_policy, self.adj_mat, batch_idle_drivers[idx]*NORMALIZATION_FACTOR, batch_demands[idx]*NORMALIZATION_FACTOR)
    ...
    def get_L(self, policies, adj_matrix, idle_drivers, demands):
