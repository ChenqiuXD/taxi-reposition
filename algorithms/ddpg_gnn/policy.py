...
class PolicyNet(nn.Module):
    def __init__(self, net_config: dict, max_bonus: float, min_bonus: float):
        ...
        self.mlp_output.add_module("sigmoid", torch.nn.Sigmoid())
    ...
    def forward(self, node_features, edge_features, edge_index):
        ...
        aggregated_message = torch.zeros([batch_mul_nodes, self.n_node, self.dim_message])
        for idx in range(edge_index.shape[1]):
            aggregated_message[edge_index[1,idx], edge_index[0,idx]%self.n_node] = message[idx]
        
        # Calculate the update results
        aggregated_message = aggregated_message.view(-1, self.dim_message*self.n_node).to(self.device)
        gnn_output = self.mlp_update(aggregated_message)

        # Calculate final output by mlp_output
        actions = (self.mlp_output(gnn_output.view(-1, self.n_node*self.dim_update)))
        actions = (actions)*(self.max_bonus-self.min_bonus)/(1-0) + self.min_bonus
        # return torch.floor(actions).detach() + actions - actions.detach()
        return actions