...
# TODO: This algorithm is used to verify the correctness of hand-calculation of ddpg gradient. Thus it should behave the same as ddpg algorithm. 


class DDPG_calc(BaseAgent):
    ...
    def learn(self):
        ...
        for idx, state in enumerate(batch_state):
            ...
            self.critic_optim.zero_grad()
            actor_criterion = self.actor(state)
            actor_criterion.retain_grad()
            critic_criteriion = - self.critic(state, actor_criterion)
            critic_criteriion.backward()

            # Calculate the comprehensive gradient
            if 'grad' in locals().keys():
                grad += actor_grads.T @ actor_criterion.grad
            else:
                grad = actor_grads.T @ actor_criterion.grad
